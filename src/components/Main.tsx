...
export class Main extends React.Component<Props, State> {
  ...
  render() {
    ...
    return (
      ...
      type="button"
      variant="primary"
      onClick={this.sendAvatarToServer}
    >
     Send Avatar to typo3
    </Button>
      </main>
    )
  }
  ...
  private sendAvatarToServer = () => {
    const svgNode = ReactDOM.findDOMNode(this.avatarRef!)! as Element;
    const data = svgNode.outerHTML;
    const svg = new Blob([data], { type: 'image/svg+xml' });
  
    const formData = new FormData();
    formData.append('avatar', svg, 'avatar.svg');
  
    fetch('https://lab3.gtn-solutions.com/avatar_rest/typo3_11/api/index.php', {
      method: 'POST',
      body: formData,
    })
      .then(response => {
        if (response.ok) {
          ...
          console.error('Error sending avatar to server');
        }
      })
      .catch(error => {
